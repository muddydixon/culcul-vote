...
export default {
  reset(eventId){
    dispatcher.dispatch({type: "STATUS_RESET", status: (status = beginStatus.replace(/:eventId/, eventId))});
  },
  forward(app, event){
    if(!event){
      dispatcher.dispatch({type: "STATUS_SET", status: (status = beginStatus)});
    ...
    if(type === "event" && st === "play"){
      ...
      // vote/:eventId/:voteId/:answerId/pause => answer/:eventId/:voteId+1/-1/play
      status = ["vote", eventId, +voteId + 1, -1, "play"].join("/");
    }else if(type === "answer" && st === "pause"){
      status = ["answer", eventId, voteId, answerId, "play"].join("/");
    }else if(type === "answer" && st === "play"){
      status = ["answer", eventId, voteId, answerId, "stop"].join("/");
    }else if(type === "answer" && st === "stop"){
    ...
    app.send(`status`, {eventId: event.id, status});
    dispatcher.dispatch({type: "STATUS_SET", status});
    return Promise.resolve(status);
  },
  backward(app, event){
    if(!event){
