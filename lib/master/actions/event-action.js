...
export default {
  ...
  deleteVote(app, eventId, voteId){;
    return new Promise((resolve, reject)=>{
      const event = events.find((e)=> e.id === eventId);
      event.votes = event.votes.filter((v)=> v.id !== voteId);
      dispatcher.dispatch({type: "DELETE_ANSWER", events: [].concat(events)});
      resolve(events);
      console.log("vote", event);
      app.send(`event/update`, {event});
    });
  ...
  summarize(vote, answerId){
    ...
    const ids = _.flattenDeep(vote.answers.map(a => a.votes.map(v => v.from)));
    console.log(ids);
