...
export default class NewVote extends Component {
  ...
  submitEasyVote(ev){
    ...
    const answers = easyType === "Count" ? ["Yes"] : easyType === "YesNo" ? ["Yes", "No"] : [" ", " ", " "];
    this.props.onSubmit(`アンケート ${easyType}`, "simultaneous", "once", "aftervote", answers);
  }
  render (){
    if(!this.state.isEditing) return <div className="container">
      ...
      <button className="btn btn-success" id="Count" onClick={this.submitEasyVote.bind(this)}><i className="fa fa-plus">&nbsp;</i>簡易質問追加 (カウント)</button>&nbsp;
      <button className="btn btn-success" id="YesNo" onClick={this.submitEasyVote.bind(this)}><i className="fa fa-plus">&nbsp;</i>簡易質問追加 (Yes/No)</button>&nbsp;
      <button className="btn btn-success" id="three" onClick={this.submitEasyVote.bind(this)}><i className="fa fa-plus">&nbsp;</i>簡易質問追加 (三択)</button>
      </div>;
