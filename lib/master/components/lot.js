...
export default class Lot extends Component {
  drawLot(){
    ...
    const winners = [];
    const winnerNum = +(this.refs.winnerNum.value.trim() || 0);
  ...
  render(){
    return <div style={{display: "inline-block"}}>
      ...
      <a style={{display: "inline-block"}} target="_blank" className="btn btn-danger" onClick={this.drawLot.bind(this)}>Lot</a>
      </div>;
  }
};
