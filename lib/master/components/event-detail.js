...
export default class EventDetail extends Component {
  ...
  ping(){
    ...
    SubAction.ping(this.props.app);
  }
  render (){
    const {events, status, broker, prefix} = this.props.data;
    const event = events.find((e)=> e.id === this.props.params.id);
    const num_exists = this.props.data.exists ? this.props.data.exists.length : 0;
    // const pongflag = this.state.pongflag;
    ...
    return <div className="container">
      ...
      {type && type === "event" ? <i className={`fa fa-${st}`}>&nbsp;</i> : null}
        ...
        {this.state.pongflag && <p style={{display: "inline-block"}}>{num_exists}</p>}
      </h1>
