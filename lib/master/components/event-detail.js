...
export default class EventDetail extends Component {
  ...
  onAddVote(title, answerType, countType, displayType){
    return EventAction.addVote(this.props.app, this.props.params.id, title, answerType, countType, displayType);
  }
  forward(){
  ...
  render (){
    ...
    return <div className="container">
      ...
      {type && type === "event" ? <i className={`fa fa-${st}`}>&nbsp;</i> : null}
        {event.title}&nbsp;
        <button className="btn btn-info" onClick={this.forward.bind(this)}>Forward</button>&nbsp;
        <a target="_blank" className="btn btn-success" href={`./slave.html?id=${event.id}&broker=${broker}&prefix=${prefix}`}>公開ページへ</a>&nbsp;
        <Lot app={this.props.app} event={event} />
      </h1>
      <Votes status={status} votes={event.votes} eventId={event.id}/>
