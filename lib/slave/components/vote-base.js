...
export default class VoteBase extends Component {
  ...
  renderResultYesNo(){
    ...
    return <div className="result-yesno">
      ...
      <div>
      <div className="bar text-left" style={{background: "red",  width: `${100 * cnts[0] / total}%` }}>{cnts[0]} ({0|100 * cnts[0] / total} %)</div>
      <div className="bar text-right" style={{background: "blue", width: `${100 * cnts[1] / total}%` }}>{cnts[1]} ({0|100 * cnts[1] / total} %)</div>
      </div>
    </div>;
  }
